...
В пространствах большой размерности все объекты примерно на одном расстоянии

Но, к счастью, на реальных данных...  
**Есть внутренняя размерность** (intrinsic), все объекты лежат около низкоразмерного многообразия

## Метрические алгоритмы: итог

**Плюсы:**

- Не требуется признаковых описаний (достаточно уметь измерять расстояния/близости)
- Легкая реализация
- Интерпретируемость
- Нет обучения
